...
<br />spid를 통해 lock 을 확인해 보면 해당 <strong>EXEC [USP_A] </strong>에 대한 Lock 을 확인할 수 있으며,&nbsp;<br />( Mode 가 X, IX의 경우)<br />내부 USP_B 의 Row Lock에 대해서는 Update를 통해 확인해 볼 수 있다.<br /><br />동시에 다수 sp가 실행되더라도 제일 처음 lock을 잡은 sp에서 조건에 대해 수행을 하면서 update를 진행하며,&nbsp;<br />나머지 sp는 해당 테이블 row에 대한 접근 대기 상태이다.<br /><br />이후 첫번째 sp가 실행 완료되게 되면 특정 조건을 update하게 되므로 나머지 sp들은 [USP_B] 의 조건에 접근하지&nbsp;<br />못하게 된다.<br /><br /><strong>결국 중첩 SP 내에서의 Lock 정상 확인!</strong>
</body>
</html>