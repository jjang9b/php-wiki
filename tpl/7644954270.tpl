...
<ol>
<li><span style="color: #333333;"><span style="color: #993300;">SCE(Strict Contextual Escaping)</span> 을 통한<span style="color: #993300;"> XSS, 클랙재킹 </span>등의 보안 대책.<br /><br /></span></li>
<li><span style="color: #333333;"><span style="color: #993300;">jqLite</span>가 내장되어 있어 기본적인 jQuery 함수 사용 가능.<br /><br /></span></li>
...
<li><span style="color: #993300;">AngularJS Hint </span><br /><span style="color: #333333;"> - 개발 모범 사례 준수 여부 체크<br /> - <a href="https://github.com/angular/angular-hint" style="color: #333333;">https://github.com/angular/angular-hint</a></span><br /><br />
...
<span style="color: #333333;">&nbsp; </span><br /><span style="color: #333333;"><strong><br />[ </strong><strong>양방향 바인딩 ]</strong> </span><br /><span style="color: #333333;"><strong><br /> </strong>AngularJS는 dirty-checking라는 방식으로 구현되어 있는데, 이것은 바인딩 되는 모든 변수에 대해 특정 시간에 이전 값과 현재 값을 비교하고 값에 변화가 있으면 DOM을 갱신하는 구조이다.</span><br /> <br /><span style="color: #333333;"> 이를 순차적으로 기술해 보면</span><br /><span style="color: #333333;"> - HTML을 분석하여 지시문(directive)을 컴파일할 때 바인딩되는 변수를 <span style="color: #993300;">$scope.$watch()로 등록</span>한다.</span><br /><span style="color: #333333;"> - $rootScope 자식들의 scope를 차례로 검색해, watch에 등록된 모든 변수의 변경 검사를 수행한다. 이 과정을 <span style="color: #993300;">$digest 루프</span>라고 부르고, 아래와 같은 타이밍에서 실행된다.(정기적인 폴링은하지 않습니다)</span><br />
<ul>
<li><span style="color: #333333;"><span style="color: #993300;">$scope.$apply()</span>를 부를 때.</span></li>
<li><span style="color: #333333;"><span style="color: #993300;">DOM 이벤트</span> (onChange, onClick 등)가 발생한 후.</span></li>
<li><span style="color: #993300;">$http와 $resource에서 응답이 돌아왔을 경우.</span></li>
